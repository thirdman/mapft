...
enableEthereum.then((accounts) => {
  ...
  /**
   * HANDLES THE LOAD OF THE VIEW PAGE
   * TODO: refacvtor to make consistent with other pages
   */
  
  const urlTokenId = getQueryVariable("token");
  const contractId = getQueryVariable("contract");
  
  if(urlTokenId){
    console.log("urlIokenId", urlTokenId);
    tokenID = urlTokenId;
    bodyElement.classList.add("hasId");
    readthatshit(tokenID);
    contentElement.classList.remove('hidden');
  } else {
    bodyElement.classList.remove("hasId");
    bodyElement.classList.add("hasSearch");
  }
...
mintButton.addEventListener('click', function mintthatshit() {
setMintStatus('confirming')
contract.createNewArt(fileIPFSHash, fileArweaveHash, fileIPFSHash, artistName.value, imageName.value, artistNote.value, exhibition.value, royaltyFee.value, totalCap.value, fileType.value, (err, result) => {
   if (!err) {
...
const readButton = document.getElementById('x');
const tokenIdInput = document.getElementById('w');
const contentElement = document.getElementById('readContent');
const bodyElement = document.getElementsByTagName('body')[0];
let tokenID = tokenIdInput.value;

readButton.addEventListener('click', function (){  
  tokenID = tokenIdInput.value;
  if(tokenID){
    bodyElement.classList.add("hasId");
    contentElement.classList.remove('hidden');
    readthatshit(tokenID);
