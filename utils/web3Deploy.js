...
const deployThatShit = (event, state, context) => {
  ...
  if (doIt) {
    ...
    console.log('contractDEPLOY', contractDEPLOY)
    ...
    contractDEPLOY.createContract(
      ...
      (err, result) => {
        if (!err) {
          ...
          context.commit('deployFormStore/setDeployStatus', 'error', err)
          console.error(err)
        }
      }
    )
...
function recursiveFunction(num, txHash, context) {
  ...
  return asyncThing(num, txHash, context).then((num, hash) => decide(num, hash))
}
